...
__all__ = (
    ...
    "HierarchicalDynamicContextAttention"
)
...
class HierarchicalDynamicContextAttention(nn.Module):
    def __init__(self, in_channels):
        super(HierarchicalDynamicContextAttention, self).__init__()

        # 特征通道卷积
        self.feature_conv = nn.Conv2d(in_channels, in_channels, kernel_size=3, padding=1)

        # 上下文卷积
        self.context_conv = nn.Conv2d(in_channels, in_channels, kernel_size=1)
    ...
    def forward(self, x):
        ...
        """
        # 提取特征信息
        feature_output = self.feature_conv(x)  # 特征通道输出

        # 提取上下文信息
        context_output = self.context_conv(x)  # 1x1卷积保持特征图尺寸

        # 对上下文信息进行全局平均池化以获得上下文特征
        context_output_pooled = F.adaptive_avg_pool2d(context_output, (1, 1))  # (B, C, 1, 1)
        context_output_pooled = context_output_pooled.view(context_output_pooled.size(0), -1)  # (B, C)

        # 拼接特征与上下文
        combined_output = torch.cat((feature_output, context_output), dim=1)  # (B, 2C, H, W)

        # 计算注意力权重
        B, C, H, W = combined_output.size()
        combined_flat = combined_output.view(B, -1, H * W)  # (B, 2C, H*W)
        attention_weights = self.sigmoid(self.fc(combined_flat))  # (B, C, H*W)

        # 对注意力权重进行重缩放
        attention_weights = attention_weights.view(B, C, 1, 1)  # (B, C, 1, 1)

        # 应用注意力权重
        output = combined_output * attention_weights  # 应用权重
